    ...
    <body>
        <section>
            <div class="row no-gutters">
                ...
                <?php $conexion = mysqli_connect("localhost", "root", "", "lobofood");
                      $ID = $_SESSION['ID'];
                      $cafe = "SELECT * FROM cafeteria WHERE ID='$ID'";
        ...
        <div class="container2">
            <div class="form-box">
                ...
                <form action="actualizar_cafe.php" method="post" class="input-group">
                <input type="text" class="input-field" name="cafeteria" placeholder="Nombre de la cafetería" <?php echo 'value="'.$nom.'"'; ?> required>
				<input type="text" class="input-field" name="dueño" placeholder="Propietario" <?php echo 'value="'.$pro.'"'; ?> required>
                <input type="password" class="input-field" name="pass" placeholder="Contraseña" minlength="4" maxlength="8" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{4,}">
                <input type="tel" class="input-field" name="tel" placeholder="Teléfono" <?php echo 'value="'.$tel.'"'; ?> pattern="[0-9]{10}" required>
                <input type="text" class="input-field" name="ubic" placeholder="Ubicación" <?php echo 'value="'.$ubi.'"'; ?> required>
                <div id="mensaje" style="display: none;" class="w3-panel w3-pale-red w3-border"></div>
                <button type="submit" name="btn_ac" class="submit-btn">Actualizar</button>
                <br><br><br>
                <button type="submit" class="submit-btn" name="btn_dc" style=" background: linear-gradient(to right,rgba(240, 15, 15, 0.8),rgba(243, 103, 22, 0.8));">Borrar Cafeteria</button>
            </form>
            </div>
        </div>
    </body>
